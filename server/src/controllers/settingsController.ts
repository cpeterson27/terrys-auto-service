...
export const getPublicAvailability = async (_req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const settings = await getAvailabilitySettings();
...
export const getAvailability = async (_req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const settings = await getAvailabilitySettings();
    res.json({
      ...settings,
